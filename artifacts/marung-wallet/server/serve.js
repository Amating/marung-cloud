/**
 * Production server for Expo static builds.
 *
 * Routing:
 * - GET / or /manifest with expo-platform header → native manifest JSON (Expo Go)
 * - Browser requests → Expo web build (static-build/web/index.html + assets)
 * - Falls back to landing page if web build is not present
 */
...
function serveWebBuild(pathname, req, res) {
...
if (webBuildReady) {
  console.log("Web build detected — browser visitors will see the web app");
} else {
...
const server = http.createServer((req, res) => {
  ...
  if (webBuildReady) {
    return serveWebBuild(pathname, req, res);
  }
...
server.listen(port, "0.0.0.0", () => {
  console.log(`Serving Marung Cloud on port ${port}`);
});
