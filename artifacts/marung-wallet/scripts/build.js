...
async function buildWebExport(domain) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (proc.stdout) proc.stdout.on("data", (d) => { const s = d.toString().trim(); if (s) console.log(`[web] ${s}`); });
    if (proc.stderr) proc.stderr.on("data", (d) => { const s = d.toString().trim(); if (s) console.error(`[web] ${s}`); });
    proc.on("close", (code) => {
      if (code === 0) { console.log("Web export complete"); resolve(); }
